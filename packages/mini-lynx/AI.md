...
```tsx
import { globalEngine, mount, pageElement, setEngine } from '@amritk/mini-lynx'
...
- **`<Show>` / `<Switch>`+`<Match>` / `<Dynamic>` / `<For>` / `<Index>`** from
  `@amritk/mini-lynx/flow` — each owns a `<wrapper>`, Lynx's grouping element
...
| Import | Purpose |
|---|---|
| `@amritk/mini-lynx` | `renderPage`, signals, `mount`, `pageElement`, `list`, binds, `setEngine` / `globalEngine`, the tree ops, JSX types |
...
```ts
import { clearEngine, mount, setEngine } from '@amritk/mini-lynx'
...
- **A recycled cell pool is per-shape.** `recycle` (on `/recycle`) drives the
  engine's cell callbacks, so ten thousand rows realise a handful of elements.
  Cells pool by `reuseIdentifier`, and rows sharing one get handed each other's
  elements — so if `cell` branches into structurally different trees, give each
  branch its own identifier. `cell` receives **getters**, not values: a cell is
  built once and re-pointed at row after row, and a closure over the row it
  happened to be built with would show row 0 forever.
- **A move inside a recycling list costs a remove plus an insert.** Correct, not
  minimal. Unchanged rows are left alone.
...
- **Reduced motion needs one line from you.** `RouteStack` consults
  `reducedMotion()` and skips the transition, and you can read it anywhere else.
  But the runtime cannot obtain the value: there is no reduced-motion field on
  `SystemInfo`, and Lynx has no media queries, so no `prefers-reduced-motion`.
  Your host app reads it natively; call `setReducedMotion` once at startup.

Three subpaths exist for the cases where there is no attribute to bind a signal
to, because the engine calls *you* or because the capability is an action rather
than state: `/recycle` (cell recycling), `/gestures` (`setGestureDetector`, and
the `waitFor` / `simultaneous` / `continueWith` relations that ordinary events
cannot express) and `/elements` (`querySelector`, `querySelectorAll`, `invoke`
for `scrollTo`, `boundingClientRect`, `scrollIntoView` and the rest).
