 ...
 * per-target answer. `@amritk/mini`'s router has the same function for the same
 * reason, which is why the two agree about what a route pattern means.
 *
 * The grammar is intentionally tiny — the only two things a real app reaches
 * for:
 * - `:name` captures one path segment into `params.name`.
 * - a trailing `*` captures the rest of the path into `params.rest` (the empty
 *   string when nothing follows), which covers nested layouts and catch-alls.
 *
 ...
 * segments are decoded so `%20` and friends arrive readable.
 */
