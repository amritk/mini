...
/**
 * A query exposed as signals. `result` is the full TanStack Query result;
...
/**
 * Bridges a `@tanstack/query-core` `QueryObserver` to signals — the data layer,
 * without this package hand-rolling a resource primitive. It mirrors how
 * `solid-query` wraps query-core: caching, deduplication, retries, and
 * invalidation all come from TanStack Query, and this file only forwards the
 ...
 *   const user = createQuery(client, () => ({ queryKey: ['user', id()], queryFn: () => fetchUser(id()) }))
 *   return (
 *     <Show when={user.data} fallback={() => <text>Loading…</text>}>
 *       {(loaded) => <text>{() => loaded().name}</text>}
 *     </Show>
 *   )
 * }
 * ```
