 ...
 * `submit`, `pointer`) are reshaped individually by each host, since only the
 * host knows where its target keeps a coordinate. The ones here carry nothing
 * portable beyond the raw event, so naming them is all that is needed: they
 * still get an object with a `raw`, which is what tells a handler it is holding
 * a framework payload rather than a platform one.
 *
 ...
 * that check under it.
 */
export const NAMED_EVENTS_WITHOUT_DATA = new Set(['focus', 'blur', 'load', 'error', 'hoverin', 'hoverout'])
