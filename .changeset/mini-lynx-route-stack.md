...
Add `RouteStack` — the navigation stack — to `@amritk/mini-lynx/router`, alongside the `RouteView` single slot it has always had.
...
`transition` is a seam and defaults to none, which is also what an app passes when it is respecting a reduced-motion preference — and the settled tree is identical either way, which is the rule the whole seam rests on. `fadeTransition` is built on an inline CSS transition, so the engine interpolates it rather than a JavaScript timeline stepping opacity; it skips itself entirely when the main-thread context has no timer to sequence the two writes with. `fadeTransition` is the only one shipped: opacity means the same thing on every target, while a slide's distance is the viewport's width and its direction depends on the writing mode, so the seam hands over both cards and the direction and lets an app write one.
