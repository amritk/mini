...
Also exported `LynxElement` and `LynxElementApi` from `@amritk/mini-native/hosts/lynx`. `createLynxHost` documents that you may "pass a fake to exercise the adapter off-device", and a fake has to satisfy `LynxElementApi` to be one — which lived on a module outside the export map, so taking up the invitation needed a cast.
