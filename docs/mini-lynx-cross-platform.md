# `@amritk/mini-lynx` — write once, run on web and native
...
```tsx
import { Button, Heading, Stack, Text } from '@amritk/mini-lynx/ui'
```
...
```ts
import { platform } from '@amritk/mini-lynx'
...
`mini-lynx`'s consumer is an app team that already owns its entire toolchain,
...
### 18.3 But a `mini-lynx` compiler costs about double what you would expect
...
**Level 3 as the ceiling for `mini-lynx`, level 2 for `mini`, and revisit level
4 only on evidence.**
...
Whatever the level, keep it a separate entry — `@amritk/mini-lynx/vite` and its
