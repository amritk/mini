...
const REPO = 'https://github.com/amritk/mini'
const BRANCH = 'main'
...
const buildIndex = (packages: Pkg[]): string => {
  ...
  lines.push('')
  lines.push(
    'mini is a pre-alpha monorepo of independently published `@amritk/*` packages. Each package ships an `AI.md` — a mental model, a minimal runnable example, and the gotchas most likely to trip up an LLM. `llms-full.txt` inlines all of them into one file.',
  )
  lines.push('')
  lines.push('## Packages')
  lines.push('')
...
const buildFull = (packages: Pkg[]): string => {
  const parts: string[] = []
  parts.push('# mini — full AI documentation bundle')
  parts.push('')
  parts.push(
